  ...
  <div>
        ...
        <template v-slot:activator>
          <v-list-item-content>
            <v-list-item-content>
              <v-list-item-title
                class="title"
              >{{(user.firstName || 'John') + ' ' + (user.lastName || 'Dow').toUpperCase()}}</v-list-item-title>
              <v-list-item-subtitle v-if="isLoggedIn">{{ user.email }}</v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item-content>
        </template>
    ...
    <v-list>
      <v-list-item-group v-model="item">
        <v-list-item v-for="(item, i) in items" :key="i">{{item.text}}</v-list-item>
      </v-list-item-group>
    </v-list>
  </div>
</template>
